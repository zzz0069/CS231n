...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
            h, cache_rnn = rnn_forward(caption_embed, h0, Wx, Wh, b)

        # temp affine
        temp_out, temp_cache_affine = temporal_affine_forward(h, W_vocab, b_vocab)
        # softmax
        loss, dx = temporal_softmax_loss(temp_out, captions_out, mask)
        
        # backward
        dx, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dx, temp_cache_affine)
        # rnn bp
        if self.cell_type == 'rnn':
