...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in xrange(num_train):
      ...
      loss += np.log(np.sum(np.exp(scores))) - correct_score
      dW[:, y[i]] -= X[i]
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  exp_sum_1 = np.sum(np.exp(scores), axis = 1)
  exp_sum = exp_sum_1.reshape(num_train, 1)
  loss += np.sum(np.log(exp_sum) - correct_score)
  
  margin = np.exp(scores) / exp_sum
  ind = np.zeros_like(margin)
  ind[np.arange(num_train), y] = 1
  dW = X.T.dot(margin - ind) 
