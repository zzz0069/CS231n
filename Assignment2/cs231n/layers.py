...
def batchnorm_backward(dout, cache):
    ...
    dx_hat = dout * gamma
    dvar = np.sum((x - mean) * dx_hat * -0.5 / (np.sqrt(var + eps))**3, axis = 0)
    sum1 = np.sum( - dx_hat / np.sqrt(var + eps), axis = 0)
    sum2 = np.sum(-2 * (x - mean), axis = 0)
    dmean =  sum1+ dvar * sum2 / N
    
    dx = dx_hat / (np.sqrt(var + eps))  + 2 * dvar * (x - mean) / N + dmean / N
...
def batchnorm_backward_alt(dout, cache):
    ...
    dmean += dvar * np.sum(-2 * (x - mean), axis=0) / N # dvar * np.sum(-2 * (x - mean), axis=0) = 0
    ...
    for i in range(out_height):
        for j in range(out_width):
            ...
            for n in range(N): #dx_pad
                sum1 = np.sum((dout[n, : , i, j])[:, None, None, None] * w, axis = 0)
                dx_pad[n, :, i*stride:i*stride+HH, j*stride:j*stride+WW] += sum1
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(out_height):
        for j in range(out_width):
            ...
            binary_mask = (x_mask == (max_mask)[:, :, None, None])
            temp_dout = (dout[:,:,i,j])[:,:,None,None]
            dx[:, :, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width] += binary_mask * temp_dout
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape #N,C,H,W to N,H,W,C
