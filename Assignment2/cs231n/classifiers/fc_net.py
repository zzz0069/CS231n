...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        aff_relu_out, affine_relu_cache = affine_relu_forward(X, W1, b1)
        out, fc_cache = affine_forward(aff_relu_out, W2, b2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers - 2):
            ...
            self.params[Wi] = weight_scale * np.random.randn(hidden_dims[i], hidden_dims[i+1])
            self.params[bi] = np.zeros(hidden_dims[i+1])
            if self.use_batchnorm:
                self.params[yi] = np.ones(hidden_dims[i+1])
                self.params[betai] = np.zeros(hidden_dims[i+1])
                       
        #last layer    
        WL = 'W'+str(self.num_layers)
        bL = 'b'+str(self.num_layers)
        self.params[WL] = np.random.normal(0, weight_scale, [hidden_dims[-1], num_classes])
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.use_batchnorm:
                hidden, cache[i] = affine_bn_relu_forward(hidden, 
                    ...
                    self.params[yi], self.params[betai], 
                    self.bn_params[i])
            else:
                hidden, cache[i] = affine_relu_forward(hidden, self.params[Wi], self.params[bi])
            if self.use_dropout:
        ...
        sum1 = np.sum(self.params['W'+str(self.num_layers)] * self.params['W'+str(self.num_layers)])
        loss +=  0.5 * self.reg * sum1
        ...
        for i in range(self.num_layers - 1, 0, -1):
            sum_i = np.sum(self.params['W'+str(i)] * self.params['W'+str(i)])
            loss += 0.5 * self.reg * sum_i
            if self.use_dropout:
